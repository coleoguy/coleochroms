...
m1 <- ggplot(dat, aes(x=rate)) +
  geom_density(aes(fill=as.factor(type),y=..density..),
               stat="density", position="identity", alpha=0.55) +
  geom_line(data=hpd1, aes(x=X, y=Y, color=as.factor(types)),
            alpha=0.75, size=1.4, lineend="round") +
...
m2 <- ggplot(dat2, aes(x=rate)) +
  geom_density(aes(fill=as.factor(type),y=..density..),
               stat="density", position="identity", alpha=0.55) +
  geom_line(data=hpd2, aes(x=X, y=Y, color=as.factor(types)),
            alpha=0.75, size=1.4, lineend="round") +
  geom_vline(xintercept=0, linetype="dashed", color="grey40", size=.5) +
  scale_color_manual(values = viridis(5)[c(1,4,3)]) +
  scale_fill_manual(values = viridis(5)[c(1,4,3)]) +
  guides(fill=guide_legend(title="parameter"),
         color="none") +
  xlab("Rate difference (per my)\n wingless-winged") +
  ggtheme
